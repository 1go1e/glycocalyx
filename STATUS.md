...
```
最後更新: 2026-07-29
最後 commit: c339d34 （§1 數字所依據的 ledger commit）
schema_version: 5
```
...
- **§2.2 五處標註的修訂單：尚未寫。** 2026-07-28 批次判定
  「腦微血管 1-3 μm」為 `unsupported`、「肺微血管 0.4-0.5 μm」為 `partial`；
  2026-07-29 B1 批次再判「腎絲球極厚」為 `contested`、「肝竇薄且多孔」為 `partial`；
  同日 B2a 批判「敗血症 30 分鐘內脫落」為 `unsupported`；
  同日 B2b 批再判「病理狀態下減少 50-90%」為 `partial`、
  「Syndecan-1 半衰期 2-8 小時」為 `unsupported`、「修復需 7-14 天」為 `partial`。
  **該單現有八處**（L103、L107、L108、L109、L110、L116、L117、L118）。
  依標註政策應各加一個標記。建議與 §5.5 敏感度／特異度的修訂單合併為一份。
  §2.2 已全部封閉，此時寫修訂單不會再有增補，是動手的好時機。
