...
〔注意〕123 → 230 為 2026-07-31 起四批抽 claim 的累計結果：
`axis` 全檔 16 條、`heparanase` §1 二十三條、§2 二十六條、§3 四十二條，
另有 31 條走追加 `source_ref` 未建新列。抽取仍在進行中，
完成率會持續下降，這是預期的——**不要為了數字而略過任何一節**。

〔注意〕§3 的淨新增（42）高於 §2（26），行數卻相近。成因是 `heparanase.md`
為每個器官複製同一套段落結構，§2 有的數值 §3 幾乎都有一個對應版本，
依 SCHEMA §2「歸屬不同即為不同條」全部各自建列。§4 預期為同一量級。
...
`source_ref` 主要出處（第一個 anchor）：`glycocalyx/v3` 99 列、`glycocalyx/heparanase` 91 列、
`intake/2026-07-28-糖萼層生理` 24 列、`mitochondria/axis` 16 列。
另有 59 列同時標示兩份以上文件。
出現在任一 anchor 的總列數：`heparanase` 103、`v3` 99、`生理` 79、`axis` 20。
...
而底下的數值並未跟著移動。§4（神經病變）的標題是「比腎臟、視網膜更敏感」，
屆時可驗證這是否為一個遞增序列。
...
**下一批：§4 糖尿病神經病變**，6 個子節（`4.1` – `4.6`），主要落在 `5.6.3`。

〔注意〕§4 是**第三個**平行樣本。§2 與 §3 已產生四組平行值
（heparanase 倍數／厚度降幅／屏障百分比／「X 的本質就是糖萼病變」句式，
見 `2026-08-01-heparanase-3.md` §3）。§4 抽取時應**三方並排**而非只與 §3 比對——
三點才看得出範圍是往同一方向漂移，還是各自獨立偏移。

〔注意〕§4 的結構比 §2、§3 多一層：`4.2` 分為「神經內微血管內皮糖萼」與
「神經內糖萼（雪旺氏細胞、軸突、周細胞表面）」兩類。**後者不是血管糖萼**，
`TOPICS.md` 的 `5.6.3` 是否涵蓋須先確認——這可能是本檔第一個真正的待建節點。

〔注意〕§4 已知的平行值：坐骨神經 heparanase 升高 30-80 倍、BNB 屏障 70-80%、
角膜神經糖萼厚度減少 40-60%、「Heparanase 過表現小鼠即使血糖正常也出現典型
糖尿病神經病變」（與 `5.6.2-dr-heparanase-overexpression-phenotype` 結構相同）。

〔注意〕`6.3-sglt2-glp1-restore` 已於第一批建立，並已在 `2.5`、`3.5` 兩度追加。
本檔 `4.5` 與 `7` 還會再出現，**應追加 `source_ref` 而非另建列**。

〔注意〕§4 開頭的接縫前言（L280-281）與 §3 同型：
「截至2025年11月，國際共識已將……」兼具時間形容詞與未具名權威，
依 SCHEMA §6（v4）改寫並把原措辭記入 `note`。§3 的處理見
`5.6.2-dr-earliest-detectable-event`，可直接沿用。
