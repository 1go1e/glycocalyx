...
**開始前的兩個前置，提案均已寫好，等人工核准**：

1. `reports/revisions/2026-07-30-schema-split-row.md`——混合宣稱列的拆列程序
2. `reports/revisions/2026-07-30-schema-nonaligned-evidence.md`——`~` 前綴

兩案皆為 `schema_version` 5 → 6。若同時核准，建議合併為單一 v6、單一 CHANGELOG
條目、單一 commit。**建議在抽 claim 之前先定案**：抽取會一次湧入 140–195 條，
其中必然有混合宣稱列與不對位文獻，程序先立好就能當場套用，不必事後回頭補。
...
**樣本數已達門檻（6 條），提案已寫（2026-07-30）**：
`reports/revisions/2026-07-30-schema-nonaligned-evidence.md`，狀態待人工核准。

建議採 `~` 前綴（不動欄位數、不影響既有解析，`schema_version` 5 → 6）。
提案中三處需特別留意，都是寫的時候才浮現的：
...
**提案已寫（2026-07-30）**：`reports/revisions/2026-07-30-schema-split-row.md`，
狀態待人工核准。建議採「原列 `superseded` + `note` 寫 `split_into:`」，
不新增 `status` 合法值。提案含兩條待拆列的執行草案，
其中 `5.4-s1p-preconditioning` 拆完後的面貌值得先看一眼：
支持它的是不相干系統裡的體外實驗，反駁它的是唯一一個真做了 IRI 的實驗——
混在一列時這個對比被「另一半查無資料」稀釋掉了。

本提案**不涵蓋** `source_ref` 誤配的移除程序（見上一則待決）。
