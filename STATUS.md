...
```
最後更新: 2026-07-29
最後 commit: (待提交) （§1 數字所依據的 ledger commit）
schema_version: 5
```
...
### 待決 — 原文中的時間形容詞與未具名權威宣稱（2026-07-29 新增，**掃描已完成**）

起因：`v3.md` §5.1 L301 句首「2025 年的薈萃分析指出」，同時是時間形容詞與未具名權威宣稱。
SCHEMA §6（v4）禁止這兩者進入 `statement`，但那條規範管 ledger，不管原文。

**全庫掃描已於 2026-07-29 完成**，報告 `reports/2026-07-29-scan-rhetoric.md`。實測規模：

| | 時間形容詞 | 未具名權威 | 無主詞措辭 | 小計 |
|---|---:|---:|---:|---:|
| `source/` 四個受追蹤檔（去重後） | 95 | 22 | 25 | **142** |
| `ledger/claims.csv` 121 列 statement | 2 | 1 | 3 | **6** |

三個改變決策形狀的結果：

1. **ledger 幾乎乾淨（6/121 = 5%），問題全在原文。**
   SCHEMA §6 在建列時是有效的；待決的真正對象是原文，而原文目前無任何規範管措辭
2. **具名學會或指引全庫 0 次。** 去除 `ADAM`／`ADAMTS` 假陽性後，
   `v3`、`heparanase`、`生理` 三檔皆為 0。全庫宣稱權威 43 次、指名 0 次——
   這比 §3.4 原本記錄的更強，不是 `heparanase.md` 特有的毛病，是整個語料庫的體質
3. **時間形容詞佔 136 處，是最大宗，且性質最急。**
   另兩類是一直錯，時間形容詞是**現在還沒錯、將來一定錯**，且沒有機制會提醒

**須決定四件事**（報告 §4 有展開，本節不重複）：
禁令是否延伸到原文；若延伸範圍到哪（`intake/` 原始存檔建議**不改**，理由同 §3.8）；
手段是標註還是改寫；ledger 那 6 列改不改。

**第四件可與前三件脫鉤，且成本為零**——六列全部不是 `verified`，
依 SCHEMA §6 可自由修訂 statement，不需新建 claim_id。
但其中三條 `2.1.1` 是同源列，措辭一致，若要改必須同批改。

〔推論〕與下方「statement 的無主詞措辭」是同一個問題的兩面——一個管 ledger、一個管原文。
掃描已把兩面一起量出來了，建議一次裁示。

---

### 待決 — statement 的無主詞措辭（2026-07-29 新增，**掃描已完成**）

SCHEMA §6 禁止「研究顯示」「一般認為」等無主詞措辭。
`2.1.1` 三條同源列的 statement 均以「部分文獻報告…」開頭，屬同型。

**掃描結果**：全庫 statement 欄只有 3 處無主詞措辭，即 `2.1.1-extreme-11um`、
`2.1.1-extreme-11um-brain-kidney` 與 `5.3-pbr-precedes-cognitive-2-3y`。
另有 `2.1.1-consensus-2-4um`（共識）與兩條含年份者，合計 6 列。清單見掃描報告 §3。

〔推論〕原本擔心的「全庫零星改寫會造成措辭不一致」在 ledger 這邊規模很小，
一次改完即可。真正的規模在原文那側（142 處），見上一條。

〔推論〕`3.3-meta2024-platelet` 與 `4.2.3-hrt-rct-2025` 的 **claim_id 本身含年份**。
SCHEMA §3 規定永不改名，故 statement 改寫後 ID 仍留著年份。這不是錯誤——
ID 是出身標記不是內容描述——但需記一筆，免得日後被誤認為不一致。
