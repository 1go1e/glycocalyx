...
## schema_version 12 — 2026-08-13

新增第 12 欄 `stale_after`，並新增 §13 定義其語意。這是 v7 以來第一次動欄位結構。

- **新增欄位 `stale_after`**（第 12 欄，依 §12 往後接，既有 11 欄的名稱、順序、語意不動）。
  ISO 8601 `YYYY-MM-DD` 或空。記錄該列的判定何時應重驗。
- **新增 §13**：適用範圍限 `TOPICS.md` 標記的註冊類節點（目前僅 `6.3.1`）；
  首次脫離 `unverified` 時寫入；三檔間隔（全域否定／唯一性 6 個月、進行中狀態 12 個月、
  已終結狀態 24 個月）；逾期**不自動改 `status`**，只提前回填順位；
  重驗後事實已變者走 `superseded` 另立新列，**不就地改寫 statement**。
- **§9 新增三項 `stale_after` 檢查**：格式、該填未填、不該填卻填。
  檢查 (2) 的條件是 `status -ne 'unverified'` 而非 `-eq 'verified'`，
  因為註冊類的 `unsupported` 常只代表登記在非英語系統，比文獻類更需複查。

- 觸發案例：`reports/revisions/2026-08-09-split-6.3.md` §2 群 A 第 3 點已指出
  「這群會過期，目前 schema 沒有任何欄位記錄這件事」，但當時只能寫成拆節點的理由。
  本次裁示拆分時一併補上欄位。
- 導入成本為零：`6.3.1` 的 20 列在導入時全部是 `unverified`，
  依 §13「首次脫離 `unverified` 時寫入」，**不需回溯回填任何一列**。
- 全表掃描確認：457 列中僅 `6.3.1` 的 20 列屬註冊類，其餘節點無此類主張。
  掃描時另發現 13 列含試驗字樣者實為**試驗結果**（已發表結果為永久事實，不過期），
  留在 `6.3`。試驗狀態與試驗結果的分界寫入 `TOPICS.md`。

---

## schema_version 11 — 2026-08-09

不改欄位結構，只增加兩項驗收條件，並更正 §9 的一段錯誤陳述。

- **§9 新增 `claim_id` 格式檢查**：須符合 §3 正規表示式，附**七列封閉豁免名單**。
  名單不得擴充——新建列一律須通過，發現新違規即為寫入錯誤，應在 commit 前改正。
- **§9 新增 `evidence` token 形式檢查**：去除 `!`／`~` 前綴後須符合 §4 三種形式之一。
  首次執行時全表 0 違規，屬預防性質。條文明載此檢查驗不到 §4 真正的風險
  （格式正確但不存在的識別碼），避免下一個 agent 誤以為 `evidence` 已被驗過。
- **§9 更正**：原文記「`claim_id` 與 `evidence` 兩者尚未出過事，但成因結構與本例相同」。
  實測後確認 `claim_id` **早在 `2.1.x` 那批就已出事**，該句為錯誤陳述，且正是下一個
  agent 判斷要不要跑檢查的依據。
- 觸發案例：2026-08-09 `4.1.3` 回填批次執行驗收時，順手補跑 §9 自己點名「有明文格式
  而無對應檢查」的兩欄，`claim_id` 得到 **7 列違規**：
  ...
  成因單一：slug 內嵌含小數點的數值，而 §3 的正規表示式在第一個 `-` 之後不允許 `.`。
- 受影響列：7 列，**均不修改**。§3 規定 `claim_id` 永不改名，這七個已進入 repo 歷史，
  其中數個已被其他列的 `note` 交叉引用。改為登記永久豁免。
- 未採「放寬正規表示式允許 slug 含 `.`」：slug 嵌數值正是問題源頭，放寬等於把源頭合法化。
- 決策：人工裁示 2026-08-09（選項 b，登記永久豁免）。提案見 `STATUS.md` §5 該條，
  本次未另寫 `reports/revisions/` 提案單——變更範圍限 §9 的驗收條件，不動欄位語意。
- 記入 `STATUS.md` §3.23（第二個實例）與 §5「已決」
- 同步檢查（§12）：目前無前端解析程式。`runbooks/backfill.md` §5 與
  `runbooks/extraction.md` 均未複製 `claim_id` 或 `evidence` 的格式定義，
  以交叉引用方式提及，故無需同步。
