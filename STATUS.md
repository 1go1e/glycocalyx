...
| 檔案 | 執行單 | 可刪 | 不刪 | 另案 |
|---|---|---:|---:|---:|
| `v3.md` | `2026-07-30-source-v3-wording.md` | 7 | 6 | — |
| `axis.md` | `2026-07-30-source-axis-wording.md` | 1 | 1 | 12 行收錄範圍不符 |
| `heparanase.md` | `2026-07-30-source-heparanase-wording.md` | 21 | 31 | 17 處接縫前言 |

原估計「約 52 處可刪」是錯的，實際 29 處。差額全部落在下面第 1 點。

**執行單帶出的四件事**：

1. **形容詞可以是斷言的時效限定，刪掉比留著更錯。**
   `heparanase` 16 處、`v3` 1 處。「目前沒有任何 heparanase 特異性抑制劑獲得核准上市」
   刪掉「目前」之後變成無時效的絕對斷言；「SGLT2i 是目前臨床已證實最強的……」
   刪掉之後變成無時效的最高級。這裡的形容詞不是修飾語，是**斷言的有效範圍**。

   判準：刪掉之後，這句話是否宣稱了比原本更多的東西？是則不刪。

   〔推論〕這與 §3.14「範圍被整段往上平移」同構——那條是數值範圍的兩端被錯置，
   這裡是斷言的**時間範圍**被整個移除。兩者都不是把某個東西寫錯，
   而是把某個東西的**邊界**弄丟了。邊界丟失比數值錯誤更難察覺，因為剩下的每個字都還是對的。

   〔推論〕反諷之處：措辭政策的出發點是「時間形容詞會隨時間自動變成偽陳述」，
   但在這些位置上，**刪掉時間形容詞才會讓句子立刻變成偽陳述**。
   同一個詞，在裝飾位置上是將來的錯，在限定位置上是現在的正確。

   〔推論〕`v3` 只中 1 處而 `heparanase` 中 16 處，差別在文體：`v3` 是機制敘述，
   `heparanase` 大量陳述藥物開發現況，而現況陳述天生需要時效限定。
   **文體決定了哪一種措辭錯誤會密集出現。**

2. **標題裡的措辭要分「裝飾註記」與「分節依據」。**
   `heparanase` 的「（2025 國際共識）」是裝飾，刪掉標題仍指涉同一節；
   `v3` L288「4.3 **新興**致病因子」與 `heparanase` L574「6.7 **2025年11月**臨床試驗現況總結」
   是分節依據，刪掉會讓標題指涉更多內容。測試方式與第 1 點是同一個：
   刪掉之後它指涉的範圍是否變大。

3. **`heparanase` 有 54% 的命中不在內文**——29% 在章節標題，25% 在段落接縫前言
   （「以下為……教授觀點，2025年11月最新……」，9 行 17 處）。

   〔推論〕STATUS §3.4 記錄「宣稱國際共識七次，未指名任何學會」時把它讀成語料庫的體質；
   分位置統計之後看得更清楚——**七次對應的正是七段問答，每段開場各一次**。
   體質診斷仍成立，但機制是接縫，不是語氣。

   接縫前言多數是**混合的**：框架句之後緊接一個有實質內容的宣稱，故不能整行刪，
   需句層切割，超出「只刪字，不重組句子」的範圍，另立提案。

   〔推論〕框架句宣告的是**發言者身分**，這不是未具名權威的變體。
   未具名權威至少指向某個外部來源（「國際共識」），身分宣告指向的是產生這段文字的模型
   被要求扮演的角色。它讀起來像出處，卻連宣稱有出處都沒有——查證上不可證偽。

4. **`axis.md` 含有其檔頭聲明「未收錄」的內容。** L126-137 共 12 行是 MitoQ 與 SS-31 的
   機制對照，而檔頭明文寫「未收錄部分：MitoQ……依 CLAUDE.md 第 6 條……不建 claim」。
   該段無節號，`source_ref` 的 `#{section}` 無值可取。執行單建議刪除，
   並提出例外原則：**有另一份完整保存者可刪，其餘一律不刪**。

**已修正**：`v3` 執行單 L77 由可刪改判為不刪（第 1 點同型），可刪數 8 → 7。
該檔屬待核准提案，依 §3.8 為必改類，直接修正未另寫更正單。

**另記詞表兩個缺口**（`2026-07-30-source-v3-wording.md` §6）：
`auth` 收「臨床標準」不收「標準方法」；`nosub` 收「研究顯示」不收「研究告訴我們」。

〔推論〕詞表掃描的角色只能是**下限**，不是清單——它給出「至少這麼多」，
