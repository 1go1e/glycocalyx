...
### 待核准 — 原文措辭規範的條文與存放位置（2026-07-30 新增）

政策層面已於 2026-07-30 全數核准（見下方「已決」）。剩下的是條文本身：
規則寫成什麼、放在哪。提案 `reports/revisions/2026-07-30-source-wording-rule.md`。

寫條文時撞到一件原本沒預期的事：**現行的原文規範已經放錯地方了。**
標註格式規範存在 `reports/revisions/2026-07-27-source-2.1.1-thickness.md` §2，
而該檔依 §3.8 屬歷史紀錄、明文不得修改——它同時是一條每批都要照著執行的活規則。
2026-07-29 補表格格式時已經撞到過一次，補的方式是在本檔 §5 另寫一段。
同一條規則現在分散在兩處，再補第三次會落到第三個地方。

提案建議新開 `runbooks/source-editing.md` 一次收攏三項規則（標註／改寫／刪除措辭），
舊處一字不動、以「收攏自何處」的形式重述。同時建議 **`schema_version` 不遞增**——
版本號是 `claims.csv` 的格式契約，原文措辭不在契約裡。

須裁示四件：選項 C 是否採用、草案條文是否照登、版本號是否不遞增、檔頭收錄基準是否納入。

---

### 待核准 — 三個受追蹤檔的措辭執行單（2026-07-30 新增）

條文核准後，各檔各寫一份執行修訂單，逐行列出改前／改後。規模：

| 檔案 | 可刪的純形容詞 | 抽 claim（年份／權威／無主詞） |
|---|---:|---:|
| `source/glycocalyx/v3.md` | 13（扣除同句命中他類者後約 9-11） | 27 |
| `source/glycocalyx/heparanase.md` | 38 | 31 |
| `source/mitochondria/axis.md` | 1 | 1 |

右欄約 58 條會進回填佇列，帳本會再長約 58 列，high 完成率會再跌一次。
**這是預期的**，與收錄 `heparanase.md` 時的預期屬同一筆帳。
