...
```
最後更新: 2026-08-02
最後 commit: 1773e55 （§1 數字所依據的 ledger commit）
schema_version: 9
topics_version: 2
```
...
〔注意〕123 → 273 為 2026-07-31 起五批抽 claim 的累計結果：
`axis` 全檔 16 條、`heparanase` §1 二十三條、§2 二十六條、§3 四十二條、§4 四十三條，
另有 34 條走追加 `source_ref` 未建新列。抽取仍在進行中，
...
〔注意〕2026-08-02 的 schema v9 只改 8 列的 `claim_type`，**`status` 分布不變**，
故「最後 commit」仍指向 1773e55。型別分布的變化記於 `ledger/CHANGELOG.md` v9 條目。
...
[ ] 5A       早期診斷（L362-424）       → 5.7 / 5.5 / 2.1.2
...
**下一批：§5 早期診斷方法，拆為 5A／5B 兩個子批**（2026-08-02 核准）。
