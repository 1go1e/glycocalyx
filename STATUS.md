...
### 待決 — 原文中的時間形容詞與未具名權威宣稱（2026-07-29 新增）

`v3.md` §5.1 L301 句首為「2025 年的薈萃分析指出」，同時是時間形容詞與未具名的權威宣稱。
SCHEMA §6（v4）禁止這兩者進入 `statement`，但那條規範管的是 ledger，不管原文。
2026-07-29 修訂單**未處理**此處，僅記錄。

須決定的是原則而非個案：原文中的這類措辭要不要納入標註或改寫的範圍。
若要，應先做一次全庫掃描（`heparanase.md` 的「國際共識」七次、`生理.md` 的「2025 年研究」
密集出現），而不是在各修訂單裡零星處理。

〔推論〕與下方「statement 的無主詞措辭」是同一個問題的兩面——
一個管 ledger、一個管原文。建議兩者一起裁示，用同一次全庫掃描解決。
