...
另有 1 條 `unsupported` 列的相關文獻只寫在 `note`，未進 `evidence`（見 §3.13）。
...
`2.2-sepsis-shedding-30min` 已於 2026-07-29 B2a 批完成（`unsupported`）。

**處理 `2.2-repair-7-14d` 前的必要動作**：先確認「重建 7-14 天」講的是哪一種修復。
B2a 批浮現的「輸注脂質體奈米載體後 30 分鐘內恢復」與 7-14 天差三個數量級，
兩者都落在糖萼修復的文獻範圍內。自然重建、外源補充、酵素消化後回復是三件事，
不先分開就會重蹈 §3.13。
...
脫落與修復的時間尺度定錨值已建立，見 `queries.md` §2.1.1。
...
B2a 批完成 `2.2-sepsis-shedding-30min`（unsupported）。
...
**現在不提案。** §11 的欄位變更成本高，目前樣本只有這一條加上 `2.1.1` 那兩條
（`Gromnicova 2016` 同樣只能寫進 `note`）。再出現三、四條同型再提。
下一個 session 遇到同型時，在此累加一行。
